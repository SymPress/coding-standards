...
final class FunctionDocBlock
{
    ...
    public static function allTags(
        ...
        return self::normalizeTags(
            self::findAllTags($tokens, $commentOpener + 1, $closeTag),
            $normalizeContent,
        );
    }
    ...
    public static function allParamTypes(File $file, int $functionPosition): array
    {
        ...
        foreach ($params as $param) {
            if (preg_match('~^([^$]+)\s*(\$\S+)~', trim($param), $matches) !== 1) {
                continue;
            }
    ...
    {
        /** @var list<array{tag: string, content: string}> $tags */
        $tags = [];
        ...
        for ($i = $start; $i < $end; $i++) {
            ...
            $tags[$key]['content'] .= $content;
        }

        return self::normalizeRawTags($tags);
    }
     ...
     */
    private static function normalizeRawTags(array $tags): array
    {
        $normalized = [];

        foreach ($tags as $tag) {
            $normalized[] = [$tag['tag'] ?? '', $tag['content'] ?? ''];
        }
