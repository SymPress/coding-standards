 ...
 *     sniff: string|null,
 *     warnings: array<int, list<string>>,
 *     errors: array<int, list<string>>,
 *     messages: array<int, list<string>>,
 *     properties: object{
 *         start: false|int,
 *         end: false|int,
...
class FixtureContentParser
{
    ...
    private function processResults(object $accumulator, string $fixturePath): array
    {
         ...
         * @var array<int, string|list<string>>|null $messages
         * @var array<int, string|list<string>>|null $warnings
         * @var array<int, string|list<string>>|null $errors
    ...
    {
        preg_match_all(self::PATTERN_TOKEN_LINE, $line, $matches, PREG_SET_ORDER);
        if (!$matches) {
            return false;
        }
        ...
        foreach ($matches as $match) {
            $targetLine = $lineNum;
            if ($match['line'] !== 'This') {
                $targetLine += $match['line'] === 'Next' ? 1 : -1;
            }

            $code = '';
            if (isset($match['code'])) {
                $matchedCode = trim($match['code']);
                if ($matchedCode) {
                    $code = $matchedCode;
                }
            }
            ...
            switch ($match['type']) {
                case 'Message':
                    $accumulator->messages[$targetLine][] = $code;
                    break;

                case 'Warning':
                    $accumulator->warnings[$targetLine][] = $code;
                    break;

                default:
                    $accumulator->errors[$targetLine][] = $code;
            }
        }
